...
# load data
spillover <- read_xlsx("./ws286_find_spillovers_ShouldYouExclSome_50bp.xlsx", col_names = T) %>% filter((StopCoordSpillOver == TRUE) & (should_exclude == FALSE)) %>% .$gene_id
...
# do 50bp -> 100bp gene extension followed by 3pUtr, Transcript, Exon extension
read_xlsx("./ws286_ext_Gene3pUtrTrans_50bp_20230422.xlsx", col_names = T) %>%
  arrange(seqname, start) %>%
  ExtGene(50) %>%
  Ext3pUtrTransExon() %>%
  write_xlsx("ws286_ext_Gene3pUtrTrans_100bp_20230422.xlsx", col_names = T)
