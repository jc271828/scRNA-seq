...
# load data
spillover <- read_xlsx("../edit_gtf/ws286_find_spillovers_ShouldYouExclSome.xlsx", col_names = T) %>% filter((StopCoordSpillOver == TRUE) & (should_exclude == FALSE)) %>% .$gene_id
...
# do 0bp -> 50bp gene extension followed by 3pUtr, Transcript, Exon extension
read_xlsx("../edit_gtf/ws286_new.xlsx", col_names = T) %>%
  arrange(seqname, start) %>%
  ExtGene(50) %>%
  Ext3pUtrTransExon() %>%
  write_xlsx("ws286_ext_Gene3pUtrTrans_50bp_20230422.xlsx", col_names = T)
