...
got <- got[str_detect(got$tissue, "_fed"), ]
got$tissue %<>% str_remove_all("_fed")

fed_total_num_cells <- sum(got$num_barcode)
expected <- read_xlsx("~/../Downloads/6h_fed_l1_CellNumPerTissue_20231006.xlsx")

# didn't get "Vblast" "Rectal" "Tblast" "Qblast" "Gblast" "Mblast" "Wblast"
setdiff(expected$tissue, got$tissue)
setdiff(got$tissue, expected$tissue)

compr <-
  merge(expected, got, by = "tissue", all = F) %>%
  mutate(expected_num_in_expt = round(cell_num/574 * fed_total_num_cells))
...
write_xlsx(compr, "fed_cell_num_correlation_20231006.xlsx")
...
ggplot(compr, aes(x = expected_num_in_expt, y = annotated_num_in_expt)) +
  ...
  labs(x = "(Proportionally) expected number of cells",
       ...
       title = "fed scRNASeq obtained ~ expected cell number correlation")
